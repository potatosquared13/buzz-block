...
with conn:
    ...
    c.execute('''CREATE TABLE IF NOT EXISTS clients
                 (identity TEXT PRIMARY KEY, name TEXT, contact TEXT, UNIQUE(identity))''')
    c.execute('''CREATE TABLE IF NOT EXISTS users
                 (identity TEXT, initial_balance REAL, blacklisted INTEGER DEFAULT 0, UNIQUE(identity), FOREIGN KEY(identity) REFERENCES clients(identity))''')
...
def insert_user(client, contact, amount):
    with conn:
        ...
        c.execute("INSERT INTO users VALUES(?, ?, 0)", (client.identity[:96], amount,))
...
def get_users():
    ...
    with conn:
        try:
            lock.acquire()
            c.execute("SELECT users.identity, clients.name, clients.contact, users.initial_balance, balances.current_balance, balances.pending_balance FROM users INNER JOIN clients ON users.identity=clients.identity INNER JOIN balances ON users.identity=balances.identity WHERE users.blacklisted=0")
            users = c.fetchall()
        finally:
            lock.release()
    return users
...
def get_blacklisted_users():
    ...
    with conn:
        try:
            lock.acquire()
            c.execute("SELECT users.identity, clients.name, clients.contact, users.initial_balance, balances.current_balance, balances.pending_balance FROM users INNER JOIN clients ON users.identity=clients.identity INNER JOIN balances ON users.identity=balances.identity WHERE users.blacklisted=1")
            users = c.fetchall()
        finally:
            lock.release()
    return users
...
def replace_id(old_identity, new_identity):
    with conn:
        try:
            old = search_user(old_identity)
            lock.acquire()
            c.execute("INSERT INTO clients VALUES(?, ?, ?)", (new_identity, old.name, old.contact,))
            c.execute("INSERT INTO users VALUES(?, ?, 0)", (new_identity, old.pending_balance,))
            c.execute("INSERT INTO balances VALUES(?, ?, ?)", (new_identity, old.pending_balance, old.pending_balance,))
            c.execute("UPDATE users SET blacklisted=1 WHERE identity=?", (old_identity,))
        finally:
            lock.release()